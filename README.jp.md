...
そして `dd_viewer`/`dd_molview` がインストールされたPython環境。
プラットフォーム固有なのは最初の2つだけ。

`dd_cview` は専用のconda env **`dd_cview`** を持つ——他の `dd_*`
プロジェクトが共有する `dd` envとは分けてあるので、どれか1つのパッケージ
を更新・再構築しても、このビルドを壊すリスクがない（逆も同様）。必要な
のは `dd_viewer`/`dd_molview` 自体と、`dd_cview_backend.py` が実際に
importするそれらの依存のサブセット（RDKit、biopython、pandas、numpy、
py3Dmol）、そして `pybind11` だけ——`dd_cview` の埋め込みバックエンドが
一切触れない、それぞれの追加GUI/Web依存（Streamlit、PySide6）は不要:
...
# dd_viewer/dd_molview自体はconda-forgeに無いので、チェックアウトから
# editableインストールする——上のconda依存が dd_cview が実際にimportする
# ものを全てカバーしているので --no-deps で良い。
